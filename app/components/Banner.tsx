...
const HeroSection: React.FC = () => {
    ...
    return (
        <div className="relative bg-gradient-to-r from-blue-900 to-teal-700 text-white overflow-hidden">
            ...
            <nav className="w-full py-4 px-6 md:px-12 flex items-center justify-between bg-transparent">
                ...
                <div className="hidden md:flex items-center space-x-4 lg:space-x-8">
                    {navLinks.map((link) => (
                       <Link key={link.title} href={link.title} className="text-md md:text-md lg:text-md xl:text-md text-gray-200 hover:text-white transition-colors">
                       {link.title}
                     </Link>
                    ))}
                </div>
                ...
                {/* Sign In Button */}
                <button className="hidden md:block px-6 py-2 rounded-lg border text-white transition-colors">
                    Sign in
                </button>
                ...
                {isMobileMenuOpen && (
                    <div className="fixed inset-0 bg-gray-900 bg-opacity-100 flex flex-col items-center justify-center space-y-6 z-50">
                        {/* Close Button */}
                        ...
                        {/* Navigation Links */}
                        {navLinks.map((link) => (
                            <Link key={link.title} href={link.href} className="text-gray-200 text-base  hover:text-white transition-colors">
                                {link.title}
                            </Link>
                        ))}
                    </div>
            ...
            <div className="relative max-w-7xl mx-auto px-6 md:px-12 pt-8 pb-20 flex flex-col md:flex-row items-center">
                ...
                <div className="w-full md:w-1/2 space-y-6">
                    <div className="inline-flex items-center space-x-2 bg-banner border animate-borderPulse rounded-full px-4 py-2">
                        <span> 🔥 </span>
            ...
            <div className="relative mx-auto px-2 md:px-12 py-8 flex items-center justify-between w-full bg-green-800">
                {/* Static view for larger devices */}
                <div className="hidden md:flex items-center justify-between w-full space-x-5">
                    <Image src="/spherule.png" alt="Spherule" className=' object-contain' width={120} height={60} />
                    <Image src="/samsung.png" alt="Samsung" className=' object-contain' width={120} height={60} />
                    <Image src="/visa.png" alt="Visa" className=' object-contain' width={120} height={60} />
                    <Image src="/amazon.png" alt="Amazon" className=' object-contain' width={120} height={60} />
                    <Image src="/paypal.png" alt="PayPal" className=' object-contain' width={120} height={60} />
                    <Image src="/alipay.png" alt="Alipay" className=' object-contain' width={120} height={60} />
                </div>

                {/* Mobile view (animation scroll effect) */}
                <div className="md:hidden overflow-hidden bg-green-800">
                    <div className="animate-scroll flex space-x-4 w-max">
